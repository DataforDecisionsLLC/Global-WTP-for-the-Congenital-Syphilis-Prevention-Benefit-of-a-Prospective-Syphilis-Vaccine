...
capture log close 
log using "$output\DALY\make_daly_loss_case_preterm_2030_2064_truncated", text replace 
...
outputs:
deaths_prevalence_ylds_preterm.dta
ylds_case_preterm_w_life_tables.dta
...
compress
save "$output\IHME\GBD 2021\NEONATAL PRETERM BIRTH\ihme_2022_to_2050_all_ages_reshaped", replace
...
use "$output\IHME\GBD 2021\NEONATAL PRETERM BIRTH\ihme_2022_to_2050_all_ages_reshaped", clear
...
compress
save "$output\IHME\GBD 2021\NEONATAL PRETERM BIRTH\ihme_2030_to_2050_single_ages", replace
...
clear 
capture erase "$output\DALY\deaths_prevalence_ylds_preterm.dta"
save          "$output\DALY\deaths_prevalence_ylds_preterm.dta", replace emptyok
...
use  "$output\IHME\GBD 2021\NEONATAL PRETERM BIRTH\ihme_2030_to_2050_single_ages", clear 
...
append using "$output\DALY\deaths_prevalence_ylds_preterm"
...
compress
save         "$output\DALY\deaths_prevalence_ylds_preterm", replace 
...
save  "$output\DALY\ylls_case_preterm_collapsed.dta", replace
...
compress 
save "$output\DALY\ylds_case_preterm_w_life_tables", replace
...
clear 
capture erase "$output\DALY\ylds_case_preterm_truncated.dta"
save          "$output\DALY\ylds_case_preterm_truncated.dta" , emptyok replace 
...
append using "$output\DALY\ylds_case_preterm_truncated.dta"
sort country birth_year age 
save         "$output\DALY\ylds_case_preterm_truncated.dta" , replace 
...
use "$output\DALY\ylds_case_preterm_truncated.dta", clear 
assert age==0 
drop age 
sum yld_case, d 

merge 1:1 country birth_year using "$output\DALY\ylls_case_preterm_collapsed.dta"
assert _m==3 
drop _m 

sort country birth_year 

save "$output\DALY\ylls_&_ylds_case_preterm_truncated.dta", replace
...
clear 
capture erase "$output\DALY\daly_loss_case_preterm_2030_2064_truncated.dta"
save          "$output\DALY\daly_loss_case_preterm_2030_2064_truncated.dta", replace emptyok
...
use if country=="`country'" using "$output\DALY\ylls_&_ylds_case_preterm_truncated.dta", clear
gen expand=0 
...
append using "$output\DALY\daly_loss_case_preterm_2030_2064_truncated.dta"
sort country birth_year 
compress 
save "$output\DALY\daly_loss_case_preterm_2030_2064_truncated.dta", replace  

end 

use "$output\DALY\ylls_&_ylds_case_preterm_truncated.dta", clear 
