...
capture log close 
log using "$output\DALY\UPPER\make_daly_loss_case_preterm_2030_2064_truncated_upper", text replace 
...
/*
inputs:
ihme_2022_to_2050_all_ages_reference_scenario_upper.dta created in: make_ihme_2022_to_2050_all_ages_upper_lower.do 
...
outputs:
daly_loss_case_preterm_2030_2064_truncated.dta
*/

////////////////////////////////////////////////////////////////////////////////
////////////////////// read in and reshape the IHME data ///////////////////////
////////////////////////////////////////////////////////////////////////////////

use "$output\IHME\GBD 2021\NEONATAL PRETERM BIRTH\ihme_2022_to_2050_all_ages_reference_scenario_upper", clear
assert unit=="Number"
assert condition=="Neonatal preterm birth"
drop unit condition
ren value number_
replace measure = lower(measure)
reshape wide number_, i(country year age sortme) j(measure) string
sort country year sortme 
isid country year age 

compress
save "$output\IHME\GBD 2021\NEONATAL PRETERM BIRTH\ihme_2022_to_2050_all_ages_reshaped_upper", replace

////////////////////////////////////////////////////////////////////////////////
///// expand to single age assuming deaths, prevalence & ylds are equally //////
////////////// distributed across single ages in an age group //////////////////
///////////////////////// compute ylds per case ////////////////////////////////
////////////////////////////////////////////////////////////////////////////////

use "$output\IHME\GBD 2021\NEONATAL PRETERM BIRTH\ihme_2022_to_2050_all_ages_reshaped_upper", clear
...
foreach var in deaths prevalence ylds {
	replace number_`var' = number_`var'/diff if diff>0
}
...
compress
save "$output\IHME\GBD 2021\NEONATAL PRETERM BIRTH\ihme_2030_to_2050_single_ages_upper", replace

////////////////////////////////////////////////////////////////////////////////
////// create lifetime trajectories of ylds per case, deaths & prevalence //////
////////////////////////////////////////////////////////////////////////////////

clear 
capture erase "$output\DALY\UPPER\deaths_prevalence_ylds_preterm.dta"
save          "$output\DALY\UPPER\deaths_prevalence_ylds_preterm.dta", replace emptyok

capture prog drop birth_dalys
prog birth_dalys
version 18.0 
set more off
set type double
...
use "$output\IHME\GBD 2021\NEONATAL PRETERM BIRTH\ihme_2030_to_2050_single_ages_upper", clear
...
replace keepme=1 if year==2050 & age>2050-`cohort' & age<100

keep if keepme==1 
drop keepme
gen birth_year=`cohort'

replace number_deaths=. if age>4

append using "$output\DALY\UPPER\deaths_prevalence_ylds_preterm"
sort country birth_year age
order country birth_year age year number_deaths number_prevalence yld_case

assert number_deaths==. if age>4

sort country birth_year age 
by country birth_year: replace year = year[_n-1] + 1 if age>2050-`cohort' & age<100 & year==2050 & _n>1
by country birth_year: assert year==year[_n-1] + 1 if _n>1
by country birth_year: assert age==age[_n-1] + 1 if _n>1

compress
save         "$output\DALY\UPPER\deaths_prevalence_ylds_preterm", replace 

end 

forvalues b = 2030/2050 {
	birth_dalys `b'
}
...
clear 
capture erase "$output\DALY\UPPER\le_preterm_$analysis" 
save          "$output\DALY\UPPER\le_preterm_$analysis" , emptyok replace 
...
append using "$output\DALY\UPPER\le_preterm_$analysis"  
sort country birth_year age 
save         "$output\DALY\UPPER\le_preterm_$analysis" , replace 
...
use  "$output\DALY\UPPER\le_preterm_$analysis" , clear
sort country birth_year age
assert age<=4
assert birth_year>=2030 & birth_year<=2050

merge 1:1 country birth_year age using "$output\DALY\UPPER\deaths_prevalence_ylds_preterm", ///
...
gen yll_discounted = le_discounted*number_deaths

collapse (sum) yll_discounted (min) birth_cases, by(country birth_year)

gen yll_case = yll_discounted/birth_cases
sum yll_case, d
...
save  "$output\DALY\UPPER\ylls_case_preterm_collapsed.dta", replace

////////////////////////////////////////////////////////////////////////////////
/////////////// map the lifetable variables to the ylds per case ///////////////
////////////////////////////////////////////////////////////////////////////////
...
merge 1:1 country birth_year age using "$output\DALY\UPPER\deaths_prevalence_ylds_preterm", ///
keepusing(country birth_year age yld_case) 
...
order country birth_year age yld_case  L survivors_l

compress 
save "$output\DALY\UPPER\ylds_case_preterm_w_life_tables", replace
...
clear 
capture erase "$output\DALY\UPPER\ylds_case_preterm_truncated.dta"
save          "$output\DALY\UPPER\ylds_case_preterm_truncated.dta" , emptyok replace 
...
use "$output\DALY\UPPER\ylds_case_preterm_w_life_tables", clear 
...
append using "$output\DALY\UPPER\ylds_case_preterm_truncated.dta"
sort country birth_year age 
save         "$output\DALY\UPPER\ylds_case_preterm_truncated.dta" , replace 
...
use "$output\DALY\UPPER\ylds_case_preterm_truncated.dta", clear 
assert age==0 
drop age 
sum yld_case, d 

merge 1:1 country birth_year using "$output\DALY\UPPER\ylls_case_preterm_collapsed.dta"
assert _m==3 
drop _m 

sort country birth_year 

save "$output\DALY\UPPER\ylls_&_ylds_case_preterm_truncated.dta", replace
...
clear 
capture erase "$output\DALY\UPPER\daly_loss_case_preterm_2030_2064_truncated.dta"
save          "$output\DALY\UPPER\daly_loss_case_preterm_2030_2064_truncated.dta", replace emptyok
...
use if country=="`country'" using "$output\DALY\UPPER\ylls_&_ylds_case_preterm_truncated.dta", clear
gen expand=0 
...
append using "$output\DALY\UPPER\daly_loss_case_preterm_2030_2064_truncated.dta"
sort country birth_year 
compress 
save "$output\DALY\UPPER\daly_loss_case_preterm_2030_2064_truncated.dta", replace  

end 

use "$output\DALY\UPPER\ylls_&_ylds_case_preterm_truncated.dta", clear 
...
use  "$output\DALY\UPPER\daly_loss_case_preterm_2030_2064_truncated.dta", clear
