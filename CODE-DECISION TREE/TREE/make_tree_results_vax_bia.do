...
capture log close 
log using "$output\DECISION TREE\TREE RESULTS\make_tree_results_vax_bia", text replace

/*
inputs:
bia_basefile_age15_cohort.dta created in: make_bia_basefile_age15_cohort.do
outputs:
tree_results_vax_bia.dta
*/

clear
capture erase "$output\DECISION TREE\TREE RESULTS\tree_results_vax_bia.dta"
save          "$output\DECISION TREE\TREE RESULTS\tree_results_vax_bia", replace emptyok
...
use "$output\DECISION TREE\BASE FILE\bia_basefile_age15_cohort", clear 
...
append using "$output\DECISION TREE\TREE RESULTS\tree_results_vax_bia"
sort country age 
compress
save         "$output\DECISION TREE\TREE RESULTS\tree_results_vax_bia", replace 
