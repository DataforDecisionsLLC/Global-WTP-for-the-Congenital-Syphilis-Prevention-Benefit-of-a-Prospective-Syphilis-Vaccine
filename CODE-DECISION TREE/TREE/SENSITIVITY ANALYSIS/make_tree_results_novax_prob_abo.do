...
capture log close 
log using "$output\DECISION TREE\TREE RESULTS\SENSITIVITY ANALYSIS\make_tree_results_novax_prob_abo", text replace

/*
inputs:
analytical_data_ihme_countries_age15_cohort_prob_abo.dta created in: make_analytical_data_ihme_countries_age15_cohort_prob_abo.do
outputs:
tree_results_novax_prob_abo.dta
*/

clear
capture erase "$output\DECISION TREE\TREE RESULTS\SENSITIVITY ANALYSIS\tree_results_novax_prob_abo.dta"
save          "$output\DECISION TREE\TREE RESULTS\SENSITIVITY ANALYSIS\tree_results_novax_prob_abo", replace emptyok
...
use "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age15_cohort_prob_abo", clear 
...
append using "$output\DECISION TREE\TREE RESULTS\SENSITIVITY ANALYSIS\tree_results_novax_prob_abo.dta"
sort country age 
compress
save         "$output\DECISION TREE\TREE RESULTS\SENSITIVITY ANALYSIS\tree_results_novax_prob_abo.dta", replace 
