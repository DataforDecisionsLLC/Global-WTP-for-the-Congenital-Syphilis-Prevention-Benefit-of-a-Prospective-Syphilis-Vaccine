...
capture log close 
log using "$output\DECISION TREE\TREE RESULTS\SENSITIVITY ANALYSIS\make_tree_results_novax_who_emtct", text replace

/*
inputs:
analytical_data_ihme_countries_age15_cohort_who_emtct.dta created in: make_analytical_data_ihme_countries_age15_cohort_who_emtct.do
outputs:
tree_results_novax_who_emtct.dta
*/

clear
capture erase "$output\DECISION TREE\TREE RESULTS\SENSITIVITY ANALYSIS\tree_results_novax_who_emtct.dta"
save          "$output\DECISION TREE\TREE RESULTS\SENSITIVITY ANALYSIS\tree_results_novax_who_emtct", replace emptyok
...
use "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age15_cohort_who_emtct", clear 
...
append using "$output\DECISION TREE\TREE RESULTS\SENSITIVITY ANALYSIS\tree_results_novax_who_emtct.dta"
sort country age 
compress
save         "$output\DECISION TREE\TREE RESULTS\SENSITIVITY ANALYSIS\tree_results_novax_who_emtct.dta", replace 
