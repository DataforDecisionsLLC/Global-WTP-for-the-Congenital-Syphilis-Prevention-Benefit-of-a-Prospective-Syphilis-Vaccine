...
capture log close 
log using "$output\DECISION TREE\BASE FILE\make_bia_basefile_age15_cohort", text replace 
...
post_acute_costs_series_5_years.dta created in: make_post_acute_costs_series_5_years.do
...
drop cost_*
drop if year>2034
save "$output\DECISION TREE\BASE FILE\bia_basefile_age15_cohort", replace
...
drop if year<2030
drop if year>2034
merge 1:1 country year using "$output\ECONOMIC DATA\DIRECT COSTS\post_acute_costs_series_5_years"
assert _m==3 
drop _m 
sort country year
...
merge 1:m country year using "$output\DECISION TREE\BASE FILE\bia_basefile_age15_cohort"
assert _m!=1 
