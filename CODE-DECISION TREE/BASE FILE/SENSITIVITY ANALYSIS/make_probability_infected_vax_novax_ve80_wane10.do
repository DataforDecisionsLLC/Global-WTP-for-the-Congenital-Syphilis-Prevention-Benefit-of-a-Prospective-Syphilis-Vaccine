...
capture log close 
log using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\make_probability_infected_vax_novax_ve80_wane10", text replace
...
by country: replace rve=.9*rve[_n-1] if time>0
gen base=.8
gen ve=(rve/100)*base
...
compress 
save "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\probability_infected_vax_novax_ve80_wane10", replace
