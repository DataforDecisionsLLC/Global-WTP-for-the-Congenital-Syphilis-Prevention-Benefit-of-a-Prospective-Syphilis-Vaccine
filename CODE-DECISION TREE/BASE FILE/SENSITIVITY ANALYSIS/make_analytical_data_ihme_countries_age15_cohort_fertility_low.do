...
capture log close 
log using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\make_analytical_data_ihme_countries_age15_cohort_fertility_low", text replace 
...
outputs:
analytical_data_ihme_countries_age15_cohort_fertility_low.dta
*/
...
save "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age15_cohort_fertility_low", replace
...
merge 1:1 country age year using "$output\FERTILITY\SENSITIVITY ANALYSIS\probability_pregnant_sensitivity_analysis", keepusing(country age year prob_preg_low)
assert _m!=1 
keep if _m==3 
drop _m 

ren prob_preg_low prob_preg
