...
capture log close 
log using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\make_analytical_data_ihme_countries_age20_cohort", text replace 
...
probability_infected_vax_novax_age20_cohort.dta created in: make_probability_infected_vax_novax_age20_cohort.do
...
////////////////////////////////////////////////////////////////////////////////
////////// keep only the age 20 cohort over the reproductive lifespan //////////
////////////////////////////////////////////////////////////////////////////////
...
// extend the data to age 49
gen expand=0 
replace expand = 10 if year==2050
expand expand 
sort country age year
by country age: replace year = year[_n-1] + 1 if expand==10 & age==40 & _n>1
by country age: replace age = age[_n-1] + 1 if expand==10 & age==40 & _n>1
...
compress 
save "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age20_cohort", replace 
...
keep if year>=2030 & year<=2059

merge 1:1 country year age using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age20_cohort"
assert _m!=2 
keep if _m==3 
drop _m 
...
use "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age20_cohort", clear 

merge 1:1 country age year using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\probability_infected_vax_novax_age20_cohort"
...
merge 1:m country year using  "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age20_cohort"
assert year > 2059 if _m==1 
...
merge 1:1 country year using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age20_cohort"
assert _m!=2
drop if year>2059
...
append using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age20_cohort"
