...
capture log close 
log using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\make_analytical_data_ihme_countries_age25_cohort", text replace 
...
probability_infected_vax_novax_age25_cohort.dta created in: make_probability_infected_vax_novax_age25_cohort.do
...
gen expand=0 
replace expand = 5 if year==2050
expand expand 
sort country age year
by country age: replace year = year[_n-1] + 1 if expand==5 & age==45 & _n>1
by country age: replace age = age[_n-1] + 1 if expand==5 & age==45 & _n>1
...
compress 
save "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age25_cohort", replace 
...
merge 1:1 country year age using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age25_cohort"
assert _m!=2 
keep if _m==3 
drop _m 
...
use "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age25_cohort", clear 

merge 1:1 country age year using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\probability_infected_vax_novax_age25_cohort"
...
merge 1:m country year using  "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age25_cohort"
assert year > 2054 if _m==1 
...
merge 1:1 country year using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age25_cohort"
assert _m!=2
drop if year>2054
...
append using "$output\DECISION TREE\BASE FILE\SENSITIVITY ANALYSIS\analytical_data_ihme_countries_age25_cohort"
